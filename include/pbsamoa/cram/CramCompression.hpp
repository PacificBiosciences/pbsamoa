...
/// \brief Decompression function: (compressed_data, raw_size) -> decompressed bytes.
/// Must produce exactly rawSize bytes.
...
public:
    ...
    /// \param methodId CRAM block method ID (0-255). Built-in CRAM IDs (0-8) cannot be
    /// overridden.
...
/// \brief Compress quality data using fqzcomp with explicit per-record metadata.
...
/// \brief Decompress a CRAM block in-place (replaces Data with uncompressed content).
void DecompressCramBlock(CramBlock& block);

/// \brief Decompress a CRAM block in-place with an optional reusable gzip context.
void DecompressCramBlock(CramBlock& block, libdeflate_decompressor* decompressor);

/// \brief Compress a CRAM block in-place for writing (replaces Data with compressed content).
void CompressCramBlock(CramBlock& block, CramBlockMethod method);

/// \brief Compress a CRAM block in-place with an optional reusable gzip context.
