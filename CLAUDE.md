...
`pbsamoa` (umbrella) · `dump` · `convert` · `chunk` · `bai-build` · `bai-query` · `zmi-build` · `zmi-query` · `bench`. Shared helpers: `CliUtils.hpp`, `ParseUtils.hpp`, `SamOutput.hpp`, `MetricUtils.hpp`.
