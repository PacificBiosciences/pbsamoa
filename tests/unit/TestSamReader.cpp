...
TEST(SamReader, NmTagParsed)
{
    ...
    for (auto rec = reader.ReadRecord(); rec.has_value(); rec = reader.ReadRecord()) {
        lastRecord = std::move(rec);
    }
    ASSERT_TRUE(lastRecord.has_value());
...
protected:
    ...
    void SetUp() override
    {
        tmpDir_ = std::filesystem::temp_directory_path() / "pbsamoa_test_sam_reader";
