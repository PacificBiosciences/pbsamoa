...
protected:
    ...
    void SetUp() override
    {
        tmpPath = std::filesystem::temp_directory_path() / "pbsamoa_cram_test.cram";
    ...
    void TearDown() override
    {
        ...
        std::filesystem::remove(tmpCraiPath);
    }
...
TEST_F(CramWriterReaderTest, FqzcompRecordsPerSliceRoundTripPreservesQualities)
{
    ...
    for (const std::int32_t recordsPerSlice : recordsPerSliceCases) {
        ...
        const std::size_t totalRecords =
            recordsPerSlice == 1000 ? static_cast<std::size_t>(1000) : static_cast<std::size_t>(37);
...
TEST_F(CramWriterReaderTest, CraiRegionQueryMappedReturnsOnlyOverlaps)
{
    ...
    for (const BamRecord& record : queried) {
        const std::int32_t recordEnd =
            (record.ReferenceEnd() > record.Pos()) ? record.ReferenceEnd() : record.Pos() + 1;
        EXPECT_EQ(record.RefId(), 0);
