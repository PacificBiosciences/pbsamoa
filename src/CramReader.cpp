    ...
    switch (type) {
        case 'A': {
            if (decodeKind == CramCodecDecodeKind::BYTE_ARRAY) {
                return RequirePayloadWidth(type, codec.DecodeByteArray(coreReader, extStore), 1);
            }
        ...
        case 's': {
            if (decodeKind == CramCodecDecodeKind::BYTE_ARRAY) {
                return RequirePayloadWidth(type, codec.DecodeByteArray(coreReader, extStore), 2);
            }
        ...
        case 'i': {
            if (decodeKind == CramCodecDecodeKind::BYTE_ARRAY) {
                return RequirePayloadWidth(type, codec.DecodeByteArray(coreReader, extStore), 4);
            }
        ...
        case 'f': {
            if (decodeKind == CramCodecDecodeKind::BYTE_ARRAY) {
                return RequirePayloadWidth(type, codec.DecodeByteArray(coreReader, extStore),
                                           sizeof(float));
            }
        ...
        case 'H':
            if (decodeKind != CramCodecDecodeKind::BYTE_ARRAY) {
                throw std::runtime_error{std::format(
                    "CramReader: tag type '{}' requires byte-array codec decode", type)};
            }
            payload = codec.DecodeByteArray(coreReader, extStore);
        ...
        if ((bamFlags & 0x4) == 0) {
            ...
            for (const auto& feature : features) {
                ...
                switch (feature.Code) {
                    ...
                    case 'I': {
                        const auto insLen =
                            feature.Length > 0 ? feature.Length
                                               : static_cast<std::int32_t>(std::size(feature.Data));
                        if (insLen > 0) {
                            writeFeatureData(currentReadPos, feature.Data);
                    ...
                    case 'N':
                        ...
                        break;
                    case 'H':
                        AppendCigarOp(cigar, CigarOpType::H,
                                      feature.Length > 0 ? feature.Length : 1);
                        break;
                    case 'P':
                        AppendCigarOp(cigar, CigarOpType::P,
                                      feature.Length > 0 ? feature.Length : 1);
                        break;
                    default:
                        break;
