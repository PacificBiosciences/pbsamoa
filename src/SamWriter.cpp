...
struct SamWriter::Impl
{
    ...
    {
        if (!stream) {
            throw std::runtime_error{"SamWriter: cannot open " + path.string()};
        }
                             ...
                             std::string_view seq, std::span<const std::uint8_t> qual)
{
    ...
    if (refId < 0) {
        ...
        buf.append(impl_->header.ReferenceName(refId));
    }
    buf += '\t';
    ...
    if (nextRefId < 0) {
        ...
        buf.append(impl_->header.ReferenceName(nextRefId));
    }
    buf += '\t';
...
{
    impl_->buf += '\n';
    impl_->stream.write(std::data(impl_->buf),
                         static_cast<std::streamsize>(std::size(impl_->buf)));
}
