...
inline std::array<std::byte, 16> ComputeMd5(std::span<const std::byte> data)
{
    ...
    const auto bitLength = std::uint64_t{std::size(data)} * 8u;
    for (std::size_t i = 0; i < 8; ++i) {
        const auto shifted = (bitLength >> (8 * i)) & 0xFFu;
    ...
    for (std::size_t chunkOffset = 0; chunkOffset < std::size(padded); chunkOffset += 64) {
        ...
        for (std::size_t i = 0; i < 16; ++i) {
            const auto base = chunkOffset + (i * 4);
            words[i] =
                static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(padded[base])) |
                (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(padded[base + 1])) << 8) |
                (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(padded[base + 2]))
                 << 16) |
                (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(padded[base + 3])) << 24);
        }
        ...
        for (std::size_t i = 0; i < 64; ++i) {
            ...
            d = c;
            c = b;
            const auto sum = a + f + MD5_TABLE[i] + words[g];
