...
std::span<const std::byte> TrimTrailingNul(std::span<const std::byte> payload)
{
    if (!std::empty(payload) && (payload.back() == std::byte{0})) {
        return payload.first(std::size(payload) - 1);
    }
    return payload;
}
...
EntryIter FindEntry(std::vector<TagMap::Entry>& entries, TagKey key)
{
    return std::ranges::find_if(entries,
                                [key](const TagMap::Entry& entry) { return entry.first == key; });
}
...
TagValue DecodeTagValueFromBamPayload(char type, std::span<const std::byte> payload)
{
    switch (type) {
        ...
        case 'Z': {
            const std::span<const std::byte> textBytes{TrimTrailingNul(payload)};
            return TagValue{
                std::string{reinterpret_cast<const char*>(textBytes.data()), std::size(textBytes)}};
        }
        case 'H': {
            const std::span<const std::byte> textBytes{TrimTrailingNul(payload)};
            return TagValue{HexString{std::string{reinterpret_cast<const char*>(textBytes.data()),
                                                  std::size(textBytes)}}};
        }
        case 'B': {
