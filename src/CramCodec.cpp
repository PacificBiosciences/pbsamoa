...
{
    if (contentId >= 0 && contentId < static_cast<std::int32_t>(FAST_BLOCK_LIMIT)) {
        const auto idx = static_cast<std::size_t>(contentId);
...
std::span<const std::byte> CramExternalBlockStore::GetBlockData(std::int32_t contentId) const
{
    ...
    auto it = blocks_.find(contentId);
    return it == blocks_.end() ? std::span<const std::byte>{}
                               : std::span<const std::byte>{it->second.data};
}
...
std::vector<std::byte> CramExternalBlockStore::TakeBlockData(std::int32_t contentId)
{
    ...
    auto it = blocks_.find(contentId);
    return it == blocks_.end() ? std::vector<std::byte>{} : std::move(it->second.data);
}
...
std::vector<std::byte> ExternalCodec::DecodeByteArray(CramBitReader& /*coreReader*/,
                                                      CramExternalBlockStore& extBlocks)
{
    const auto len = extBlocks.ReadItf8(blockContentId_);
    const auto view = extBlocks.ReadBytesView(blockContentId_, len);
    return {view.begin(), view.end()};
}
...
void HuffmanCodec::BuildCodes()
{
    ...
    std::ranges::sort(entries_, [](const auto& a, const auto& b) {
        return a.bitLength != b.bitLength ? a.bitLength < b.bitLength : a.symbol < b.symbol;
    });
...
std::vector<std::byte> ByteArrayLenCodec::DecodeByteArray(CramBitReader& coreReader,
                                                          CramExternalBlockStore& extBlocks)
{
    const auto len = lenCodec_->DecodeInt(coreReader, extBlocks);
    ...
    if (const auto* ext = dynamic_cast<const ExternalCodec*>(dataCodec_.get())) {
        const auto view = extBlocks.ReadBytesView(ext->BlockContentId(), len);
        return {view.begin(), view.end()};
    }
                                                                  ...
                                                                  CramExternalBlockStore& extBlocks)
{
    ...
    if (const auto* ext = dynamic_cast<const ExternalCodec*>(dataCodec_.get())) {
        return extBlocks.ReadBytesView(ext->BlockContentId(), len);
    }

    // Slow path: per-byte decode into scratch buffer
    viewScratch2_.resize(len);
    for (std::int32_t i = 0; i < len; ++i) {
